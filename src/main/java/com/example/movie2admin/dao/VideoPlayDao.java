...
public interface VideoPlayDao extends JpaRepository<VideoPlay, Long>, CrudRepository<VideoPlay, Long> {
    ...
    @Query(value = "SELECT COUNT(*)  FROM (SELECT (vs.video_time / v.vod_duration * 100) as c FROM `video_scale` AS vs INNER JOIN video AS v ON v.id=vs.video_id WHERE vs.video_id=:id AND vs.video_time > 30) AS s WHERE s.c > 0", nativeQuery = true)
    Long getScale(long id);
}
